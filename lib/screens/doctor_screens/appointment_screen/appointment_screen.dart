...
Widget listOfAppointment(DoctorViewAppointmentSuccessState state) =>
    ListView.separated(
      itemBuilder: (context, index) =>
          appointmentItemBuilder(context, state, index),
      separatorBuilder: (context, index) => myDivider(),
      itemCount: state.appointments.length,
    );
Widget appointmentItemBuilder(BuildContext context,
        DoctorViewAppointmentSuccessState state, int index) =>
    Padding(
                      ...
                      defButton(
                        function: () {
                          navigateTo(
                              context,
                              AddSessionScreen(
                                appointmentId: state.appointments[index].id,
                              ));
                        },
                        text: 'add session',
                        radius: 25.0,
...
class AddSessionScreen extends StatelessWidget {
  ...
  AddSessionScreen({super.key, required this.appointmentId});
